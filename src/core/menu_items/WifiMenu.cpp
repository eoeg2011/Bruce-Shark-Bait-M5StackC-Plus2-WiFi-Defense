...
void WifiMenu::optionsMenu() {
    ...
    if (isWebUIActive) {
        ...
        padprintln("");
        padprintln("Al iniciar una funciÃ³n WiFi, la WebUI puede dejar de funcionar");
        padprintln("");
        padprintln("Sel: continuar");
        padprintln("Cualquier tecla: MenÃº");
        while (1) {
    ...
    if (!wifiConnected) {
        ...
        if (WiFi.getMode() == WIFI_MODE_STA) options.push_back({"Info AP", displayAPInfo});
    }
    options.push_back({"Ataques WiFi", wifi_atk_menu});
    options.push_back({"Portal Maligno", [=]() {
    ...
    options.push_back({"Sniffers", [=]() {
        std::vector<Option> snifferOptions;
            ...
            snifferOptions.push_back({"Sniffer Raw",    sniffer_setup});
            snifferOptions.push_back({"Sniffer Probe",  karma_setup});
            snifferOptions.push_back({"AtrÃ¡s",      [=]()  {optionsMenu(); }});
...
void WifiMenu::configMenu() {
    options = {
        {"Agregar WiFi Maligno",    addEvilWifiMenu         },
        {"Eliminar WiFi Maligno", removeEvilWifiMenu      },
